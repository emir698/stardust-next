...
function sendSlack(webhookUrl: string, message: string): Promise<void> {
...
export const gunlukRapor = onSchedule(
  { schedule: '59 23 * * *', timeZone: 'Europe/Istanbul', secrets: [slackWebhookUrl] },
  async () => {
    ...
    await sendSlack(slackWebhookUrl.value(), mesaj);
